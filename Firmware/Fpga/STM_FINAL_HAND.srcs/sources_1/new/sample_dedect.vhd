...
entity sample_dedect is
port (
  i_clk_ref            : in  std_logic;
  i_clk_test           : in  std_logic;
  i_rstb               : in  std_logic;
  o_clock_freq         : out std_logic_vector(15 downto 0);
  o_clock_freq_int     : out INTEGER range 0 to 2500 := 0
  );
end sample_dedect;
architecture rtl of sample_dedect is
...
--signal o_clock_freq_int  : INTEGER range 0 to 65635;  
begin
  ...
  if(i_rstb='0') then
    ...
    if(r2_counter_test_strobe='1') then
      o_clock_freq         <= std_logic_vector(r2_counter_test);
		o_clock_freq_int  <= to_integer(unsigned(std_logic_vector(r2_counter_test)));
    end if;
  end if;
